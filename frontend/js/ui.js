let startCoords = { lat: 0, lon: 0 }; let endCoords = { lat: 0, lon: 0 }; let debounceTimer = null; window.currentFlotta = [];
let costChartInstance = null; // A diagram tárolására

// ÚJ: Valós idejű kapcsolat a szerverrel (Socket.io)
const socket = (typeof io !== 'undefined') ? io('http://localhost:3000') : null;
if (socket) {
  socket.on('adat_frissites', () => {
    console.log('🔄 Valós idejű frissítés érkezett a szerverről!');
...
// ÚJ: Grafikon (Statisztika) Rajzoló Függvény
async function renderDashboard() {
  ...
  try {
    const utak = await API.fetchUtak(token, true, ''); // Szűrő nélkül, az összeset lekérjük a statisztikához
    if (!Array.isArray(utak)) return;

    // Kiszámoljuk a havi költségeket
    const haviKoltsegek = {};
    utak.forEach(u => {
      if (u.status === 'TELJESITVE') {
        haviKoltsegek[u.honap_ev] = (haviKoltsegek[u.honap_ev] || 0) + u.koltseg;
      }
    });

    const labels = Object.keys(haviKoltsegek).sort();
    const data = labels.map(l => haviKoltsegek[l]);
    ...
    if (costChartInstance) {
      ...
      costChartInstance = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [{ label: 'Üzemanyag Költség (RON)', data: data, backgroundColor: 'rgba(59, 130, 246, 0.5)', borderColor: '#3b82f6', borderWidth: 1, borderRadius: 4 }]
        },
        options: {
...
async function renderUI() {
  ...
  if (currentUser.role === 'ADMIN') {
    ...
    try { const riasztasok = await API.fetchAlerts(token); if(Array.isArray(riasztasok) && riasztasok.length > 0 && alertContainer) { alertContainer.innerHTML = riasztasok.map(r => `<div class="bg-red-900/50 border-l-4 border-red-500 p-4 rounded text-red-200 text-sm font-semibold mb-3 flex items-center shadow-lg"><span class="mr-3 text-xl">⚠️</span> ${r}</div>`).join(''); } } catch (e) { }
    loadAktivFlotta();
    renderDashboard(); // Frissítjük a grafikont is
  } else { 
    document.getElementById('adminSection').classList.add('hidden'); document.getElementById('kliensHeader').classList.remove('hidden'); if (soforUrlap) soforUrlap.classList.remove('hidden'); 
  }
  ...
  // Kiszedtük az 5 másodperces setInterval-t, mert a Socket.io megcsinálja!
  loadAutok(); loadUtak(); 
}
...
async function loadAktivFlotta() {
  const token = window.AppState.token; const currentUser = window.AppState.user; if (!token || currentUser.role !== 'ADMIN') return;
  const container = document.getElementById('aktivFlottaList'); if(!container) return;
  try {
    const aktivUtak = await API.fetchAktivFlotta(token);
    if (!Array.isArray(aktivUtak) || aktivUtak.length === 0) { container.innerHTML = '<div class="text-sm text-slate-400 py-2">Jelenleg nincs kint a flotta egyetlen autója sem.</div>'; return; }
    container.innerHTML = aktivUtak.map(u => `<div class="bg-slate-900 p-3 rounded mb-2 border border-blue-900/50 flex justify-between items-center"><div><div class="font-bold text-white text-sm flex items-center gap-2">🚘 ${u.auto_rendszam} <span class="text-[9px] bg-blue-900/80 text-blue-300 px-1.5 py-0.5 rounded tracking-widest uppercase">Úton</span></div><div class="text-xs text-slate-400 mt-1">Sofőr: <span class="text-blue-400 font-semibold">${u.sofor_nev}</span></div></div><div class="text-right"><div class="text-xs text-slate-300">${u.indulas} <span class="text-blue-500">➔</span> ${u.erkezes}</div></div></div>`).join('');
  } catch(e) {}
}
...
async function loadUtak() {
  ...
  const szuro = document.getElementById('kerelmekSzuro'); const kivalasztottHonap = szuro ? szuro.value : '';
  try { 
    const utak = await API.fetchUtak(token, currentUser.role === 'ADMIN', kivalasztottHonap); if (!Array.isArray(utak)) return; 
    if(utak.length === 0) { document.getElementById('utList').innerHTML = '<tr><td colspan="7" class="py-4 text-center text-slate-500 text-sm">Nincs találat.</td></tr>'; return; }
    document.getElementById('utList').innerHTML = utak.map(u => {
      let gombHTML = '-';
      if(currentUser.role === 'ADMIN' && u.status === 'BEERKEZO') { gombHTML = `<div class="flex gap-2"><button onclick="biralUtFizikai(${u.id}, 'JOVAHAGYOTT')" class="bg-emerald-600 hover:bg-emerald-500 text-white px-2 py-1 rounded text-xs">✔</button><button onclick="biralUtFizikai(${u.id}, 'ELUTASITOTT')" class="bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded text-xs">✖</button></div>`; } 
      else if (currentUser.role === 'USER' && u.status === 'JOVAHAGYOTT') { gombHTML = `<button onclick="openLezarModal(${u.id}, '${u.auto_rendszam}')" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs font-bold w-full shadow-lg shadow-blue-500/20">⛽ Lezárás</button>`; }
      let statusColor = 'bg-amber-900/50 text-amber-400'; if(u.status === 'JOVAHAGYOTT') statusColor = 'bg-emerald-900/50 text-emerald-400 border-emerald-500/30 border'; if(u.status === 'ELUTASITOTT') statusColor = 'bg-red-900/50 text-red-400'; if(u.status === 'TELJESITVE') statusColor = 'bg-blue-900/50 text-blue-300';
      return `<tr><td class="py-3 text-slate-400">#${u.id}</td><td class="py-3 text-blue-400 font-mono text-xs font-semibold">${u.honap_ev}</td><td class="py-3 font-bold text-white">${u.sofor_nev}</td><td class="py-3"><span class="bg-slate-900 px-2 py-1 rounded text-xs">${u.auto_rendszam}</span></td><td class="py-3 text-slate-300">${u.indulas} ➔ ${u.erkezes} (${u.tavolsag} km)<br/><span class="text-[10px] text-slate-500">Költség: ${u.koltseg} RON | Fogy.: ${u.fogyasztas}L</span></td><td class="py-3"><span class="text-xs px-2 py-1 rounded font-bold ${statusColor}">${u.status}</span></td><td class="py-3">${gombHTML}</td></tr>`;
    }).join(''); 
  } catch(e) {}
}
